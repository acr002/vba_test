...
'--------------------------------------------------------------------------------------------------'
    ...
    ws_mainmenu.Select
step00:
    tabinst_fn = InputBox("作成する集計設定ファイルのファイル名を" & vbCrLf & "入力してください。" & vbCrLf & vbCrLf & "【例】A01030TN.xlsx など", "MCS 2020 - 集計設定ファイルの作成")
    If tabinst_fn = "" Then
...
'2019.10.9 - 追加処理---------------------------------------------------
    ws_setup.Select
    face_qcode = ""
    face_qcode = InputBox("表側に設定するQCODEを入力してください。" & vbCrLf & vbCrLf & "【例】F01 など" & vbCrLf & "※未入力の場合は、単純集計の集計設定となります。", "MCS 2020 - 集計設定ファイルの作成")
...
'-----------------------------------------------------------------------
    ...
    Application.StatusBar = "集計設定ファイル 作成中..."
...
' ここから集計設定ファイル作成のコーディング(´･ω･`)
                ...
                Select Case Left(ws_setup.Cells(t_r, 9).Value, 1)
                    Case "S", "M", "L"  ' SA,MA,LMAはQCODEを表頭に出力
                        ...
                        If Left(q_data(d_index).q_format, 1) = "S" Then
                            If q_data(d_index).ct_count = 1 Then
                                ' １カテゴリーは横グラフとして作成
                                ws_tabinst.Cells(t_seq + 6, 25).Value = "2"    ' 横グラフ
                            ElseIf q_data(d_index).ct_count <= 5 Then
    ...
    ws_tabinst.Cells(2, 9).Value = "【ウエイトバック集計】"
...
' システムログの出力
    ...
    If (Len(ws_mainmenu.Cells(41, 6)) > 70) Or (Len(ws_mainmenu.Cells(41, 6)) = 0) Then
      ws_mainmenu.Cells(41, 6) = "21"
    Else
      ws_mainmenu.Cells(41, 6) = ws_mainmenu.Cells(41, 6) & " > 21"
    End If
    ...
    MsgBox "集計設定ファイル " & tabinst_fn & " の作成が完了しました。", vbInformation, "MCS 2020 - Tabulation_Setting"
End Sub
